...
def modificar_estructura(tiempo: str) -> str:
    ...
    if "-" and "T" and "." and ":" in lista_tiempo:
        ...
        lista_tiempo.remove(":")
        lista_tiempo.remove(":")
        if "0" in lista_tiempo[4]: # si el mes tiene 0 lo elimino
            lista_tiempo.pop(4)
    ...
    for nombres_local, modificacion_local in dict_local.items():
        for nombres_drive, modificacion_drive in dict_drive.items():
            if nombres_local == nombres_drive:  # busca entre los dos diccionarios nombres iguales
                if modificacion_local > modificacion_drive[0]:  # lo q buscaba hacer era si uno era mayor q el otro
                    # se elimine el q tiene meno y subir el q tenia mayor fecha de modific.
                    subir_modific_drive(nombres_local, modificacion_drive[1])
                elif modificacion_local < modificacion_drive[0]: # aca lo mismo q lo anterior
